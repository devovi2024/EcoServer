          ...
          <AnimatePresence>
            {loading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {products.map((p) => {
                  const hasDiscount = p.discount && p.discount !== "false";
                  return (
                    <motion.div
                      key={p._id}
                      layout
                    ...
                    >
                      <Link to={`/details/${p._id}`} className="flex flex-col">
                        <div className="relative w-full h-48 flex items-center justify-center bg-orange-50 rounded-xl overflow-hidden">
                          <img
                            src={p.img1 || "https://via.placeholder.com/150"}
                            alt={p.title || "Product"}
                            className="object-contain h-40 w-40 group-hover:scale-105 transition-transform duration-300"
                          />
                        </div>
                        <div className="mt-4 space-y-1">
                          <h3 className="text-sm font-semibold text-gray-800 truncate">{p.title || "Unnamed Product"}</h3>
                          <p className="text-xs text-gray-500 line-clamp-2">{p.shortDescription || ""}</p>
                        </div>
                        <div className="mt-3 flex items-center gap-2">
                          {hasDiscount && <p className="text-sm text-gray-400 line-through">${p.price}</p>}
                          <p className="text-lg font-bold text-orange-600">${p.discountedPrice || p.price}</p>
                        </div>
                      </Link>
